...
RUN if [ "${ROS_NUMBER}" = "2" ]; then \
        sudo chmod +x /sbin/update_bashrc_ros_2 ; \
        sudo chown ${USER} /sbin/update_bashrc_ros_2 ; \
        echo 'echo "source '${PWDR}'/install/setup.bash" >> ~/.bashrc' >> /sbin/update_bashrc_ros_2 ; \
        cat /sbin/update_bashrc_ros_2 ; \
        sync ; \
        /bin/bash -c /sbin/update_bashrc_ros_2 ; \
        sudo rm /sbin/update_bashrc_ros_2 ; \
    fi
