...
This class works (has the observation of) for the policy FishingRodPos_X_001_pos_new_2_vel.pth
"""
...
class InferenceController(Node):
    def __init__(self):
        ...
        self.q_dot_scale = params['task']['env']['control']['qDotScale']
        ...
        self._acc_scale = params["task"]["env"]["learn"]["accLinTipScale"]
        ...
        self.acc_tip_lin = [0.0 for _ in range(3)]
        self.acc_tip_lin_previous = [0.0 for _ in range(3)]
    ...
    def imu_callback(self, msg):
        ...
        self.acc_tip_lin = linear_acceleration
    ...
    def inference_callback(self):
        ...
        obs_list = np.concatenate((      
            ...
            np.array([list(self.joint_vel.values())[0]], dtype=float).reshape((self.njoint, 1)) / self.q_dot_scale,
            np.fromiter(list(self.tip_vel_lin[0]), dtype=float).reshape((1, 1)) / self.velocity_scale,
            np.fromiter(list(self.tip_pos[0]), dtype=float).reshape((1, 1)) / self.position_scale,
            np.fromiter(list(self.tip_pos_previous[0]), dtype=float).reshape((1, 1)) / self.position_scale,
            np.fromiter(list(self.tip_vel_lin_previous[0]), dtype=float).reshape((1, 1)) / self.velocity_scale,
        ...
        self.acc_tip_lin_previous = self.acc_tip_lin.copy()
